...
| [`@constructive-io/brand-3d`](./packages/3d)               | Quad mesh → OBJ + MTL: watertight `shell` (culled internal faces, shared vertices) or per-cube `cubes` topology |
...
- [`assets/generated/`](./assets/generated) — regenerated by `pnpm generate` and checked in CI. Colorways, render
  modes, every isometric plane, the cube alphabet, OBJ/MTL for the mark and the cube wordmark (`shell` and `-cubes` topologies), and stills plus
